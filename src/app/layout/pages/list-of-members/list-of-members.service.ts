...
import {ActivityValuesInterface} from "@layout/pages/list-of-members/activity-ranking/interfaces/activity-values-interface";
import {
...
} from "@layout/pages/list-of-members/list-of-kase-members/interfaces/members-values-interface";
...
export class ListOfMembersService {
  ...
  public diplomaValues: any[] = [
    {
      ...
      items: [
        {
          header: 'На рынке корпоративных облигаций',
  ...
  requestOrganizationsFull() {
    ...
    this.http.get<any>(environment.apiUrl + 'organisations/full').subscribe({
      next: data => {
        console.log(data);
      },
      error: error => {
  ...
  requestOrganizations() {
    ...
    this.http.get<any>(environment.apiUrl + 'organisations').subscribe({
      next: data => {
        this.memberValues = {
          orders: data.orders,
          page: data.page,
